...
export default function HivSessions() {
  ...
  const actions = [
    { label: "Resources", icon: "book-open" as any, color: "#db2777", onPress: () => router.push("/(moses-hiv-support)/resources") },
                ...
                {(item as any).status === "active" && (
                  <>
                    <TouchableOpacity
                      ...
                      onPress={() => router.push({ pathname: "/(moses-hiv-support)/session-detail" as any, params: { sessionId: (item as any).id, studentName: (item as any).student?.name ?? "Patient", topic: (item as any).topic, status: (item as any).status } })}
                    >
